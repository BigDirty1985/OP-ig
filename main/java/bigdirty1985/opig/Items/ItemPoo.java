...
public class ItemPoo extends Item implements ItemModelProvider {
	...
	@Override
	public ItemPoo setCreativeTab(CreativeTabs tab) {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos,
		...
		if (!playerIn.canPlayerEdit(pos.offset(facing), facing, stack)) {
			...
			EnumDyeColor enumdyecolor = EnumDyeColor.BROWN;
				...
				if (block == Blocks.LOG && iblockstate.getValue(BlockOldLog.VARIANT) == BlockPlanks.EnumType.JUNGLE) {
					if (facing != EnumFacing.DOWN && facing != EnumFacing.UP) {
						...
						if (worldIn.isAirBlock(pos)) {
							IBlockState iblockstate1 = Blocks.COCOA.getStateForPlacement(worldIn, pos, facing, hitX,
									hitY, hitZ, 0, playerIn, stack);
